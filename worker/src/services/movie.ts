...
export const persistMovie = async (data: any) => {
    ...
    try {
        if (data.belongs_to_collection) {
            ...
            const existedCollection = await collectionRepository.findOne(
                collection.id
            );
...
export const fetchThenPersistData = async (id: string) => {
    const { data } = await axios.get(`${process.env.DATA_URI}/movie/${id}`, {
        params: {
            api_key: process.env.DATA_KEY,
        },
    });
    await persistMovie(data);
    console.log('Done persist:', id);
};
